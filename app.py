...
fig = px.scatter(merged_df, x='count', y='volume', hover_data=['country'], title='Correlation between Dinosaur Bones Count and Oil Production Volume')

# Add trendline to show potential correlation
fig.update_traces(marker=dict(size=12))
fig.update_layout(shapes=[
    dict(
      type='line',
      yref='y', y0=merged_df['volume'].min(), y1=merged_df['volume'].max(),
      xref='x', x0=merged_df['count'].min(), x1=merged_df['count'].max()
    )
])
